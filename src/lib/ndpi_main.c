...
flow_is_proto(struct ndpi_flow_struct *flow, u_int16_t p) {
...
static u_int8_t ndpi_domain_level(const char *name) {
    ...
    while ((c = *name++) != '\0') {
        ...
        state = ndpi_domain_level_automat[state][(u_int8_t)
        c];
        if (!state) break;
    }
...
*/
static int
ndpi_remove_host_url_subprotocol(struct ndpi_detection_module_struct *ndpi_str, char *value, int protocol_id) {
...
static void init_string_based_protocols(struct ndpi_detection_module_struct *ndpi_str) {
    ...
    if (!ndpi_xgrams_inited) {
        ...
        ndpi_xgrams_init(ndpi_str, impossible_bigrams_bitmap, sizeof(impossible_bigrams_bitmap),
                         ndpi_en_impossible_bigrams,
                         sizeof(ndpi_en_impossible_bigrams) / sizeof(ndpi_en_impossible_bigrams[0]), 2);
...
int ndpi_set_detection_preferences(struct ndpi_detection_module_struct *ndpi_str, ndpi_detection_preference pref,
    ...
    switch (pref) {
        case ndpi_pref_direction_detect_disable:
            ndpi_str->direction_detect_disable = (u_int8_t)
            value;
            break;
...
static void ndpi_init_protocol_defaults(struct ndpi_detection_module_struct *ndpi_str) {
    ...
    ndpi_set_proto_defaults(ndpi_str, 1 /* cleartext */, 0 /* nw proto */, NDPI_PROTOCOL_UNRATED, NDPI_PROTOCOL_UNKNOWN,
                            ...
                            ndpi_build_default_ports(ports_a, 0, 0, 0, 0, 0) /* TCP */,
                            ndpi_build_default_ports(ports_b, 0, 0, 0, 0, 0) /* UDP */);
    ndpi_set_proto_defaults(ndpi_str, 1 /* cleartext */, 0 /* nw proto */, NDPI_PROTOCOL_UNSAFE,
                            NDPI_PROTOCOL_FTP_CONTROL,
                            "FTP_CONTROL", NDPI_PROTOCOL_CATEGORY_DOWNLOAD_FT,
                            ndpi_build_default_ports(ports_a, 21, 0, 0, 0, 0) /* TCP */,
                            ndpi_build_default_ports(ports_b, 0, 0, 0, 0, 0) /* UDP */);
    ndpi_set_proto_defaults(ndpi_str, 1 /* cleartext */, 0 /* nw proto */, NDPI_PROTOCOL_ACCEPTABLE,
    ...
    ndpi_set_proto_defaults(ndpi_str, 1 /* cleartext */, 0 /* nw proto */, NDPI_PROTOCOL_ACCEPTABLE,
                            ...
                            ndpi_build_default_ports(ports_b, 0, 0, 0, 0, 0) /* UDP */);
    ndpi_set_proto_defaults(ndpi_str, 0 /* encrypted */, 1 /* app proto */, NDPI_PROTOCOL_SAFE,
    ...
    ndpi_set_proto_defaults(ndpi_str, 0 /* encrypted */, 1 /* app proto */, NDPI_PROTOCOL_ACCEPTABLE,
    ...
    ndpi_set_proto_defaults(ndpi_str, 1 /* cleartext */, 1 /* app proto */, NDPI_PROTOCOL_UNSAFE, NDPI_PROTOCOL_MINING,
                            ...
                            ndpi_build_default_ports(ports_b, 0, 0, 0, 0, 0) /* UDP */);
    ndpi_set_proto_defaults(ndpi_str, 1 /* cleartext */, 1 /* app proto */, NDPI_PROTOCOL_ACCEPTABLE,
    ...
    ndpi_set_proto_defaults(ndpi_str, 1 /* cleartext */, 1 /* app proto */, NDPI_PROTOCOL_FUN, NDPI_PROTOCOL_XBOX,
                            ...
                            ndpi_build_default_ports(ports_a, 0, 0, 0, 0, 0) /* TCP */,
                            ndpi_build_default_ports(ports_b, 0, 0, 0, 0, 0) /* UDP */);
    ndpi_set_proto_defaults(ndpi_str, 1 /* cleartext */, 1 /* app proto */, NDPI_PROTOCOL_FUN,
    ...
    ndpi_set_proto_defaults(ndpi_str, 1 /* cleartext */, 1 /* app proto */, NDPI_PROTOCOL_FUN,
                            ...
                            ndpi_build_default_ports(ports_a, 0, 0, 0, 0, 0) /* TCP */,
                            ndpi_build_default_ports(ports_b, 0, 0, 0, 0, 0) /* UDP */);
    ndpi_set_proto_defaults(ndpi_str, 0 /* encrypted */, 1 /* app proto */, NDPI_PROTOCOL_POTENTIALLY_DANGEROUS,
    ...
    ndpi_set_proto_defaults(ndpi_str, 1 /* cleartext */, 0 /* nw proto */, NDPI_PROTOCOL_FUN, NDPI_PROTOCOL_WARCRAFT3,
                            ...
                            ndpi_build_default_ports(ports_a, 0, 0, 0, 0, 0) /* TCP */,
                            ndpi_build_default_ports(ports_b, 0, 0, 0, 0, 0) /* UDP */);
    ndpi_set_proto_defaults(ndpi_str, 1 /* cleartext */, 0 /* nw proto */, NDPI_PROTOCOL_FUN,
    ...
    ndpi_set_proto_defaults(ndpi_str, 1 /* cleartext */, 0 /* nw proto */, NDPI_PROTOCOL_ACCEPTABLE, NDPI_PROTOCOL_SAP,
                            ...
                            ndpi_build_default_ports(ports_b, 0, 0, 0, 0,
                                                     0) /* UDP */); /* Missing dissector: port based only */
    ...
    ndpi_set_proto_defaults(ndpi_str, 0 /* encrypted */, 1 /* app proto */, NDPI_PROTOCOL_ACCEPTABLE,
                            ...
                            ndpi_build_default_ports(ports_b, 0, 0, 0, 0, 0) /* UDP */);
    ndpi_set_proto_defaults(ndpi_str,0 /* encrypted */, 1 /* app proto */, NDPI_PROTOCOL_ACCEPTABLE,
                            NDPI_PROTOCOL_TELEGRAM_CALL,
                            "TelegramCALL", NDPI_PROTOCOL_CATEGORY_CHAT,
                            ndpi_build_default_ports(ports_a, 0, 0, 0, 0, 0) /* TCP */,
                            ndpi_build_default_ports(ports_b, 0, 0, 0, 0, 0) /* UDP */);
    ndpi_set_proto_defaults(ndpi_str, 0 /* encrypted */, 1 /* app proto */, NDPI_PROTOCOL_ACCEPTABLE, NDPI_PROTOCOL_RUBIKA_CALL,
    ...
    ndpi_set_proto_defaults(ndpi_str, 0 /* encrypted */, 1 /* app proto */, NDPI_PROTOCOL_ACCEPTABLE, NDPI_PROTOCOL_BALE,
                            ...
                            ndpi_build_default_ports(ports_a, 0, 0, 0, 0, 0) /* TCP */,
                            ndpi_build_default_ports(ports_b, 0, 0, 0, 0, 0) /* UDP */);
    ndpi_set_proto_defaults(ndpi_str, 0 /* encrypted */, 1 /* app proto */, NDPI_PROTOCOL_ACCEPTABLE, NDPI_PROTOCOL_BALE_FILE,
                            "BALE_FILES", NDPI_PROTOCOL_CATEGORY_CHAT,
                            ndpi_build_default_ports(ports_a, 0, 0, 0, 0, 0) /* TCP */,
                            ndpi_build_default_ports(ports_b, 0, 0, 0, 0, 0) /* UDP */);
    ndpi_set_proto_defaults(ndpi_str, 0 /* encrypted */, 1 /* app proto */, NDPI_PROTOCOL_ACCEPTABLE, NDPI_PROTOCOL_BALE_MESSAGE,
                            "BALE_MESSANGER", NDPI_PROTOCOL_CATEGORY_CHAT,
                            ndpi_build_default_ports(ports_a, 0, 0, 0, 0, 0) /* TCP */,
                            ndpi_build_default_ports(ports_b, 0, 0, 0, 0, 0) /* UDP */);
    ndpi_set_proto_defaults(ndpi_str, 0 /* encrypted */, 1 /* app proto */, NDPI_PROTOCOL_ACCEPTABLE, NDPI_PROTOCOL_ETTA,
                            "ETTA", NDPI_PROTOCOL_CATEGORY_CHAT,
                            ndpi_build_default_ports(ports_a, 0, 0, 0, 0, 0) /* TCP */,
                            ndpi_build_default_ports(ports_b, 0, 0, 0, 0, 0) /* UDP */);
    ...
    ndpi_set_proto_defaults(ndpi_str, 1 /* cleartext */, 0 /* nw proto */, NDPI_PROTOCOL_ACCEPTABLE,
                            ...
                            ndpi_build_default_ports(ports_b, 5355, 0, 0, 0,
                                                     0) /* UDP */); /* Missing dissector: port based only */
    ...
    ndpi_set_proto_defaults(ndpi_str, 1 /* cleartext */, 0 /* nw proto */, NDPI_PROTOCOL_ACCEPTABLE, NDPI_PROTOCOL_NOE,
                            ...
                            ndpi_build_default_ports(ports_a, 0, 0, 0, 0, 0) /* TCP */,
                            ndpi_build_default_ports(ports_b, 0, 0, 0, 0, 0) /* UDP */);
    ndpi_set_proto_defaults(ndpi_str, 0 /* encrypted */, 0 /* nw proto */, NDPI_PROTOCOL_ACCEPTABLE,
    ...
    ndpi_set_proto_defaults(ndpi_str, 1 /* cleartext */, 0 /* nw proto */, NDPI_PROTOCOL_ACCEPTABLE,
                            ...
                            ndpi_build_default_ports(ports_b, 5001, 5201, 0, 0,
                                                     0) /* UDP */); /* Missing dissector: port based only */
    ndpi_set_proto_defaults(ndpi_str, 0 /* encrypted */, 1 /* app proto */, NDPI_PROTOCOL_FUN,
...
static int ndpi_add_host_ip_subprotocol(struct ndpi_detection_module_struct *ndpi_str,
    ...
    */
    ...
    if ((node = add_to_ptree(ndpi_str->protocols_ptree, AF_INET, &pin, bits)) != NULL) {
        ...
        for (i = 0; i < UV16_MAX_USER_VALUES; i++) {
            if (node->value.u.uv16[i].user_value == 0) {
                node->value.u.uv16[i].user_value = protocol_id, node->value.u.uv16[i].additional_user_value = htons(
                        port);
    ...
    if (ndpi_str != NULL) {
      ...
      nbpf_free(ndpi_str->nbpf_custom_proto[i].tree);
        ...
        if (ndpi_str->custom_categories.ipAddresses_shadow != NULL)
            ndpi_patricia_destroy((ndpi_patricia_tree_t *) ndpi_str->custom_categories.ipAddresses_shadow,
                                  free_ptree_data);
                ...
                if (flow) {
                    ...
                    if (packet->payload_packet_len < sizeof(struct ndpi_icmphdr)) {
                        ...
                        if (packet->payload_packet_len > sizeof(struct ndpi_icmphdr)) {
                            ...
                            u_int16_t chksm = ndpi_calculate_icmp4_checksum(packet->payload,
...
int ndpi_handle_ipv6_extension_headers(u_int16_t l3len, const u_int8_t **l4ptr,
    ...
    while (l3len > 1 &&
           (*nxt_hdr == 0 || *nxt_hdr == 43 || *nxt_hdr == 44 || *nxt_hdr == 60 || *nxt_hdr == 135 || *nxt_hdr == 59)) {
...
static u_int8_t ndpi_detection_get_l4_internal(struct ndpi_detection_module_struct *ndpi_str, const u_int8_t *l3,
    ...
    if (iph != NULL && ndpi_iph_is_valid_and_not_fragmented(iph, l3_len)) {
        ...
        if (ndpi_handle_ipv6_extension_headers(l3_len - sizeof(struct ndpi_ipv6hdr), &l4ptr, &l4len, &l4protocol) !=
            0) {
            return (1);
        }
...
static int ndpi_init_packet(struct ndpi_detection_module_struct *ndpi_str,
   ...
   */
    ...
    l4_result =
            ndpi_detection_get_l4_internal(ndpi_str,(
    const u_int8_t *) decaps_iph, l3len, &l4ptr, &l4len, &l4protocol, 0);
    ...
    if (l4protocol == IPPROTO_TCP) {
        ...
        if (l4_packet_len >= packet->tcp->doff * 4) {
            ...
            packet->payload = ((u_int8_t * )
            packet->tcp) +(packet->tcp->doff * 4);
...
void ndpi_connection_tracking(struct ndpi_detection_module_struct *ndpi_str,
            ...
            if ((ndpi_str->input_info == NULL)
                ...
                if (tcph->syn != 0 && tcph->ack == 0 && flow->l4.tcp.seen_syn == 0
                    ...
                    if (tcph->syn != 0 && tcph->ack != 0 && flow->l4.tcp.seen_syn == 1
                        ...
                        if (tcph->syn == 0 && tcph->ack == 1 && flow->l4.tcp.seen_syn == 1 &&
                            flow->l4.tcp.seen_syn_ack == 1 &&
                            flow->l4.tcp.seen_ack == 0) {
	 ...
	 */
	  ...
	  */
        ...
        if (ndpi_is_multi_or_broadcast(packet)); /* multicast or broadcast */
        else {
            if (flow->packet_direction_complete_counter[flow->client_packet_direction] == 0)
                ndpi_set_risk(ndpi_str, flow, NDPI_UNIDIRECTIONAL_TRAFFIC,
                              "No client to server traffic"); /* Should never happen */
    ...
    */
        ...
        case NDPI_PROTOCOL_ANYDESK:
            if (flow->l4_proto == IPPROTO_TCP) /* TCP only */
                ndpi_set_risk(ndpi_str, flow, NDPI_DESKTOP_OR_FILE_SHARING_SESSION,
                              "Found AnyDesk"); /* Remote assistance */
            break;
...
int ndpi_search_into_bittorrent_cache(struct ndpi_detection_module_struct *ndpi_struct,
    ...
    if (ndpi_struct->bittorrent_cache) {
        ...
        found =
                ndpi_lru_find_cache(ndpi_struct->bittorrent_cache, key, &cached_proto,
                                    0 /* Don't remove it as it can be used for other connections */,
                                    ndpi_get_current_time(flow))
                || ndpi_lru_find_cache(ndpi_struct->bittorrent_cache, key1, &cached_proto,
                                       0     /* Don't remove it as it can be used for other connections */,
                                       ndpi_get_current_time(flow))
                || ndpi_lru_find_cache(ndpi_struct->bittorrent_cache, key2, &cached_proto,
                                       0     /* Don't remove it as it can be used for other connections */,
                                       ndpi_get_current_time(flow));
...
static void ndpi_check_tcp_flags(struct ndpi_detection_module_struct *ndpi_str,
    ...
    if ((flow->l4.tcp.cli2srv_tcp_flags & TH_SYN)
        ...
        ndpi_set_risk(ndpi_str, flow, NDPI_TCP_ISSUES, "Connection refused (server)");
             ...
             && (flow->all_packets_counter <
                 5 /* Ignore connections terminated by RST but that exchanged data (3WH + RST) */)
            )
        ndpi_set_risk(ndpi_str, flow, NDPI_TCP_ISSUES, "Connection refused (client)");
    else if ((flow->l4.tcp.srv2cli_tcp_flags & TH_RST) &&
             (flow->packet_direction_complete_counter[1 /* server -> client */] == 1))
...
ndpi_protocol ndpi_detection_giveup(struct ndpi_detection_module_struct *ndpi_str, struct ndpi_flow_struct *flow,
    ...
    *protocol_was_guessed = 0;
    ...
    if ((flow->guessed_protocol_id == NDPI_PROTOCOL_STUN) ||
        ...
        ndpi_set_detected_protocol(ndpi_str, flow, NDPI_PROTOCOL_STUN, NDPI_PROTOCOL_UNKNOWN,
                                   NDPI_CONFIDENCE_DPI_PARTIAL);
    ...
    if (ret.app_protocol == NDPI_PROTOCOL_UNKNOWN &&
        ...
        ndpi_set_detected_protocol(ndpi_str, flow, NDPI_PROTOCOL_BITTORRENT, NDPI_PROTOCOL_UNKNOWN,
    ...
    if (ret.app_protocol == NDPI_PROTOCOL_UNKNOWN &&
        ...
        ndpi_set_detected_protocol(ndpi_str, flow, NDPI_PROTOCOL_ZOOM, NDPI_PROTOCOL_UNKNOWN,
                                   NDPI_CONFIDENCE_DPI_PARTIAL_CACHE);
        ret.app_protocol = flow->detected_protocol_stack[0];
    }
    ...
    if (ret.app_protocol == NDPI_PROTOCOL_UNKNOWN &&
        ...
        ndpi_set_detected_protocol(ndpi_str, flow, NDPI_PROTOCOL_OOKLA, NDPI_PROTOCOL_UNKNOWN,
                                   NDPI_CONFIDENCE_DPI_PARTIAL_CACHE);
        ret.app_protocol = flow->detected_protocol_stack[0];
    }
...
static int
ndpi_do_guess(struct ndpi_detection_module_struct *ndpi_str, struct ndpi_flow_struct *flow, ndpi_protocol *ret) {
    struct ndpi_packet_struct *packet = &ndpi_str->packet;
...
static ndpi_protocol ndpi_internal_detection_process_packet(struct ndpi_detection_module_struct *ndpi_str,
    ...
    if ((!flow->risk_checked)
        ...
        if (packet->udp)
            found = ndpi_get_guessed_protocol_id(ndpi_str, IPPROTO_UDP,
                                                 ntohs(flow->c_port),
                                                 ntohs(flow->s_port)),
                    default_ports = ndpi_str->proto_defaults[ret.master_protocol ? ret.master_protocol
                                                                                 : ret.app_protocol].udp_default_ports;
        else if (packet->tcp)
            found = ndpi_get_guessed_protocol_id(ndpi_str, IPPROTO_TCP,
                                                 ntohs(flow->c_port),
                                                 ntohs(flow->s_port)),
                    default_ports = ndpi_str->proto_defaults[ret.master_protocol ? ret.master_protocol
                                                                                 : ret.app_protocol].tcp_default_ports;
        else
	...
	*/
                ...
                if (!found) {
                    ndpi_default_ports_tree_node_t *r = ndpi_get_guessed_protocol_id(ndpi_str, packet->udp ? IPPROTO_UDP
                                                                                                           : IPPROTO_TCP,
                                                                                     ntohs(flow->c_port),
                                                                                     ntohs(flow->s_port));
...
void ndpi_parse_packet_line_info(struct ndpi_detection_module_struct *ndpi_str, struct ndpi_flow_struct *flow) {
    ...
    for (a = 0;
         ((a + 1) < packet->payload_packet_len) && (packet->parsed_lines < NDPI_MAX_PARSE_LINES_PER_PACKET); a++) {
...
ndpi_get_flow_category(struct ndpi_detection_module_struct *ndpi_str, struct ndpi_flow_struct *flow) {
...
ndpi_protocol ndpi_guess_undetected_protocol_v4(struct ndpi_detection_module_struct *ndpi_str,
    ...
    if ((proto == IPPROTO_TCP) || (proto == IPPROTO_UDP)) {
        ...
        if (rc != NDPI_PROTOCOL_UNKNOWN) {
            ret.app_protocol = rc,
                    ret.master_protocol = ndpi_guess_protocol_id(ndpi_str, flow, proto, sport, dport,
                                                                 &user_defined_proto);
...
int ndpi_check_dga_name(struct ndpi_detection_module_struct *ndpi_str,
	 ...
	 */
                    || ((max_domain_element_len >= 19 /* word too long. Example bbcbedxhgjmdobdprmen.com */) &&
                        ((num_char_repetitions > 1) || (num_digits > 1)))
                    ) {
                if (flow) {
                    ndpi_set_risk(ndpi_str, flow, NDPI_SUSPICIOUS_DGA_DOMAIN, name);
                }
	    ...
	    */
                    ...
                    if ((num_trigram_dots < 2) && (word[i + 2] != '\0')) {
                        ...
                        if (ndpi_is_trigram_char(word[i]) && ndpi_is_trigram_char(word[i + 1]) &&
                            ndpi_is_trigram_char(word[i + 2])) {
                            if (trigram_char_skip) {
                                ...
                                /* Count vowels */
                                num_trigram_vowels += ndpi_is_vowel(word[i]) + ndpi_is_vowel(word[i + 1]) +
                                                      ndpi_is_vowel(word[i + 2]);
                            }
                        } else {
                            if (word[i] == '.')
                                num_trigram_dots++;
            ...
            if (ndpi_verbose_dga_detection)
                printf("[%s][num_found: %u][num_impossible: %u][num_digits: %u][num_bigram_checks: %u][num_vowels: %u/%u][num_trigram_vowels: %u][num_trigram_found: %u/%u][vowels: %u][rc: %u]\n",
                       name, num_found, num_impossible, num_digits, num_bigram_checks, num_vowels, len,
                       num_trigram_vowels,
                       num_trigram_checked, num_trigram_found, num_vowels, rc);

            if ((len > 16) && (num_dots < 3) && ((num_vowels * 4) < (len - num_dots))) {
                if ((num_trigram_checked > 2) && (num_trigram_vowels >= (num_trigram_found - 1))); /* skip me */
                else
                    rc = 1;
            }
