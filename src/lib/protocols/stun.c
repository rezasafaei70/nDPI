...
static ndpi_int_stun_t ndpi_int_check_stun(struct ndpi_detection_module_struct *ndpi_struct,
    ...
    for (int i = 0; i < packet->payload_packet_len; i++) {
        ...
        if (packet->payload[i] == 0x72 && packet->payload[i + 1] == 0x75 && packet->payload[i + 2] == 0x62
            && packet->payload[i + 3] == 0x69 && packet->payload[i + 4] == 0x6b && packet->payload[i + 5] == 0x61
            && packet->payload[i + 6] == 0x2e && packet->payload[i + 7] == 0x69 && packet->payload[i + 8] == 0x72
                ) {
            *app_proto = NDPI_PROTOCOL_RUBIKA_CALL;
            return (NDPI_IS_STUN);
        }

        if (packet->payload[i] == 0x74 && packet->payload[i + 1] == 0x65 && packet->payload[i + 2] == 0x6c
            && packet->payload[i + 3] == 0x65 && packet->payload[i + 4] == 0x67 && packet->payload[i + 5] == 0x72
            && packet->payload[i + 6] == 0x61 && packet->payload[i + 7] == 0x6d
                ) {
            *app_proto = NDPI_PROTOCOL_TELEGRAM_CALL;
            return (NDPI_IS_STUN);
        }
    }
